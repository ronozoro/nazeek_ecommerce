...
import { Menu, MenuItem, Divider, Sidebar, Pagination, Segment, Image, Loader, Grid, GridColumn, GridRow, Button, Select, Dropdown } from 'semantic-ui-react'
...
render() {
      ...
      const pageNumbers = [];
      for (let i = 1; i <= Math.ceil(this.props.Products.length / itemsPerPage); i++) {
        pageNumbers.push(i);
      }
...
<Grid.Row>
      {
            ...
            <Menu.Item as='a' onClick={this.nextPage} icon>
              <Icon name='chevron left' />
            </Menu.Item>
               {renderPageNumbers}           
            <Menu.Item as='a' icon>
              <Icon name='chevron right' onClick={this.prevPage} />
            </Menu.Item>
          </Menu> 
      </div>
}
